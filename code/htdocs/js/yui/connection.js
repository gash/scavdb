		...
		var poll = window.setInterval(
			function(){
				if(o.conn.readyState==3){
					...
					responseObject = oConn.createResponseObject(o, callback.argument);
					if(callback.interactive){
						if(!callback.scope){
							callback.interactive(responseObject);
						}
						else{
							callback.interactive.apply(callback.scope, [responseObject]);
						}
					}
				}
